...
export async function sendPasswordResetEmail(toEmail, firstName, resetToken) {
  ...
  const encodedData = Buffer.from(combinedData).toString('base64');
  const resetUrl = `${baseUrl}/api/reset-password-page?data=${encodedData}`;
  
  // Fallback URL
  const encodedToken = encodeURIComponent(resetToken);
  const encodedEmail = encodeURIComponent(normalizedEmail);
  const fallbackUrl = `${baseUrl}/api/reset-password-page?token=${encodedToken}&email=${encodedEmail}`;
