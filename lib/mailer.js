...
export async function sendVerificationEmail(toEmail, firstName, verificationToken) {
  ...
  // Also create a base64 encoded version as a fallback (some email clients strip query params)
  const base64Token = Buffer.from(verificationToken).toString('base64');
  const base64Email = Buffer.from(toEmail).toString('base64');
  const fallbackUrl = `${baseUrl}/api/verify-email?t=${base64Token}&e=${base64Email}`;
  ...
  try {
    const result = await resend.emails.send({
      ...
      html: `
        ...
        <p style="word-break: break-all; font-family: monospace; font-size: 12px; background: #f5f5f5; padding: 10px; border-radius: 4px;">${verificationUrl}</p>
